...
class UserService:
    ...
    def register_user(
        ...
        """
        if len(senha) > 8:
            return False, "O campo senha deve ter no mínimo 8 caracteres."
        
        if len(email) < 10 or "@" not in email not in email.endswith(".com"):
            return False, "O formato do email deve conter mínimo de 10 caracteres, conter '@' e terminar com '.com'"
